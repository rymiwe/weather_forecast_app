...
class OpenWeatherMapClient < ApiClientBase
    ...
    Rails.cache.fetch(cache_key, expires_in: weather_cache_ttl) do
      ...
      if normalized_address.match?(/^\d{5}(-\d{4})?$/)
        ...
        if result && result["lat"] && result["lon"]
          ...
          # Fallback to direct geocoding if zip code doesn't work
          Rails.logger.info("Zip API failed, falling back to direct geocoding")
          return fallback_direct_geocoding("#{zip}, US")
        end
      else
  ...
  # Fallback to direct geocoding when zip lookup fails
  # @param query [String] The query to geocode
  # @return [Hash, nil] The geocoding result or nil if not found
  def fallback_direct_geocoding(query)
    Rails.logger.info("Using fallback direct geocoding for: #{query}")
    result = fetch_from_api("geo/1.0/direct", { q: query, limit: 1 })
    
    if result.is_a?(Array) && result.any?
      Rails.logger.info("Fallback geocoding result: #{result.first.inspect}")
      return result.first
    end
    
    Rails.logger.warn("Fallback geocoding returned no results for: #{query}")
    nil
  end
